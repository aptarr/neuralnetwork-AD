...
def main(argv):
	...
	try:
		...
		if sys.argv[1] != "training" and sys.argv[1] != "predicting" and sys.argv[1] != "testing" and sys.argv[1] != "counting":
			...
			hidden_layers = [int(x) for x in sys.argv[5].split(",")]
...
def rnnids(phase = "training", filename = "", protocol="tcp", port="80", type = "rnn", hidden_layers = [32, 16], seq_length = 3, dropout = 0.0, testing_filename = "", batch_size = 1):
	global done
...
def init_model(type, hidden_layers, seq_length, dropout):
	...
	#if type == "rnn":
	#	rnn_model.add(SimpleRNN(units=hidden_layers[-1], input_shape=(seq_length, embed_len)))
	#elif type == "lstm":
	#	rnn_model.add(LSTM(units=hidden_layers[-1], input_shape=(seq_length, embed_len)))
	#elif type == "gru":
	#	rnn_model.add(GRU(units=hidden_layers[-1], input_shape=(seq_length, embed_len)))
