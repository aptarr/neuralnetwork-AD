...
def main(argv):
    try:
        ...
        # traceback.print_exc()
        print(
            "Usage: python converting.py <tcp|udp> <port> <seq_lenght> <training filename>"
        )
...
def count_and_convert(phase, protocol, port, testing_filename, seq_length):
    global done
    print("masuk")
    check_directory("texts_new")

    filename_output = os.path.basename(
        testing_filename
    )  # Get the last part of the path
    filename_output = os.path.splitext(filename_output)[0]  # Remove the .pcap extension

    print("fileoutput = " + filename_output)
    print("filename = " + testing_filename)

    fmean = open("texts_new/{}_{}_{}.txt".format(filename_output, phase, port), "w")
    if phase == "training":
        for elem in byte_seq_generator(
            phase, testing_filename, protocol, port, seq_length
        ):
            fmean.write("{}\n".format(elem))
    else:
        for id, elem in byte_seq_generator(
            phase, testing_filename, protocol, port, seq_length
        ):
            fmean.write("{}; {}\n".format(id, elem))
    fmean.close()
...
def get_pcap_file_fullpath(filename):
    ...
    for i in range(0, len(conf["root_directory"])):
        # print(f"root_directory: {conf['root_directory'][i] + filename}")
        # print(f"is file: {os.path.isfile(conf['root_directory'][i] + filename)}")
        if os.path.isfile(conf["root_directory"][i] + filename):
            print(
                f"file {i} file exists: {os.path.isfile(conf['root_directory'][i] + filename)}"
            )
