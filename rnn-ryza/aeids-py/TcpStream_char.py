...
class TcpStream(threading.Thread):
    ...
    def get_payload(self, dest):
        if dest == "client":
            client_chars = [chr(byte) for byte in self.client_data] 
            return client_chars
        elif dest == "server":
            server_chars = [chr(byte) for byte in self.server_data]
            return server_chars
